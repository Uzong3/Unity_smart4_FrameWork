...
public class GalleryControl : ControlBase<GalleryControl>
{
    int nowStoryID;
    ...
    protected override void Start()
    {
        ...
        btn_Return.onClick.AddListener(() => SoundManager.Instance.PlaySFX((int)ENVI.BUTTON_SFX));
        ...
        btn_Setting.onClick.AddListener(() => SoundManager.Instance.PlaySFX((int)ENVI.BUTTON_SFX));

        btn_Next.onClick.AddListener(ClickNextButton);
        btn_Return.onClick.AddListener(ReturnMenu);
        btn_Log.onClick.AddListener(() => UIControl.Instance.ActiveLog(true));
        btn_Setting.onClick.AddListener(() => UIControl.Instance.ActiveSetting(true));
    ...
    public void Initialize(int _startID, int _endID)
    {
        endID = _endID;
    ...
    public void ClickNextButton()
    {
        ...
        if (data.Nextid == 0 || data.ID == endID)
        {
            if(cor_Auto != null) StopCoroutine(cor_Auto);
            if (cor_StoryEffect != null) StopCoroutine(cor_StoryEffect);
            GameManager.Instance.ChangeScene(SCENE.TITLE, () => { TitleControl.Instance.Initialize(); UIControl.Instance.GalleryPopup.Initialize(); });
        }
        else
        {
    ...
    {
        StoryData data = SData.GetStoryData(nowStoryID);
    ...
    void ReturnMenu()
    {
        UIControl.Instance.MessagePopup.Initialize(
            () => 
            {
                if(cor_Auto != null) StopCoroutine(cor_Auto);
                if(cor_StoryEffect != null) StopCoroutine(cor_StoryEffect);
                GameManager.Instance.ChangeScene(SCENE.TITLE, UIControl.Instance.GalleryPopup.Initialize); 
            },
             SData.GetLocalizeEnviData((int)LOCALIZE.RETURN),
             SData.GetLocalizeEnviData((int)LOCALIZE.RETURN_LOG), 
             SData.GetLocalizeEnviData((int)LOCALIZE.RETURN)
            );
    }
